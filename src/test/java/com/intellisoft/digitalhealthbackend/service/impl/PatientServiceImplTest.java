...
class PatientServiceImplTest {
    ...
    void setUp() {
        ...
        testObservation = Observation.builder()
                .code("BP-001")
                .value("120/80")
        ...
        testObservationWrapper = new ObservationWrapper(
                "BP-001",
                "120/80",
                "2024-01-15 10:30:00"
        );
    ...
    void createPatient() {
        PatientWrapper patientWrapper = PatientWrapper.builder()
                .identifier(34477307L)
                .gender("MALE")
                .familyName("Doe")
                .givenName("John")
                ...
                .build();
        when(patientRepository.findByIdentifierAndSoftDeleteFalse(34477307L)).thenReturn(Optional.empty());
        when(patientRepository.save(any(Patient.class))).thenAnswer(invocation -> invocation.getArgument(0));
        UniversalResponse response = patientServiceImpl.createPatient(patientWrapper);
        verify(patientRepository, times(1)).findByIdentifierAndSoftDeleteFalse(34477307L);
    ...
    void retrievePatient() {
        when(patientRepository.findByIdAndSoftDeleteFalse(patientId)).thenReturn(Optional.of(testPatient));
    ...
    void deletePatientWithEncounters() {
        ...
        when(patientRepository.findByIdAndSoftDeleteFalse(patientId)).thenReturn(Optional.of(testPatient));
        PatientException exception = assertThrows(PatientException.class, () -> patientServiceImpl.deletePatient(patientId));
    ...
    void addPatientEncounters() {
        ...
        EncounterWrapper encounterWrapper = EncounterWrapper.builder()
        ...
        UniversalResponse response = patientServiceImpl.addPatientEncounters(patientId, encounterWrapper);
    ...
    void endPatientEncounter() {
        ...
        PatientException exception = assertThrows(
    ...
    void retrievePatientEncountersAndObservation_Success() {
        int page = 0;
        int size = 10;
        Encounter firstEncounter = createEncounter(1L, testPatient, LocalDateTime.now().minusDays(1));
        Encounter secondEncounter = createEncounter(2L, testPatient, LocalDateTime.now().minusDays(3));
        ...
        when(patientRepository.findPatientByFamilyNameAndGivenNameAndIdentifierAndBirthDateAndSoftDeleteFalse(
                family, givenName, identifier, formatDate(birthDate)))
                .thenReturn(Optional.of(testPatient));
        when(encounterRepository.findByPatientIdAndSoftDeleteFalse(patientId, PageRequest.of(page, size)))
                .thenReturn(encounterPage);
        UniversalResponse response = patientServiceImpl.retrievePatientEncountersAndObservation(
    ...
    void viewPatientObservations_Success_WithObservations() {
        Observation obs1 = createObservation(1L, "BP", "120/80", testPatient, encounter, LocalDateTime.now().minusDays(1));
        Observation obs2 = createObservation(2L, "Temperature", "37.5", testPatient, encounter, LocalDateTime.now().minusDays(2));
        Observation obs3 = createObservation(3L, "Heart Rate", "75", testPatient, encounter, LocalDateTime.now().minusDays(3));
        ...
        when(patientRepository.findByIdAndSoftDeleteFalse(patientId))
                .thenReturn(Optional.of(testPatient));
        when(observationRepository.findObservationByPatientAndSoftDeleteFalse(testPatient, pageable))
    ...
    void addEncounterObservationWhenEncounterExists() {
        ...
        UniversalResponse response = patientServiceImpl.addEncounterObservation(1L, testObservationWrapper);
    ...
    void addEncounterObservationWhenEncounterNotFound_ShouldThrowPatientException() {
        when(encounterRepository.findByIdAndSoftDeleteFalse(anyLong())).thenReturn(Optional.empty());
        PatientException exception = assertThrows(PatientException.class, () -> {
            patientServiceImpl.addEncounterObservation(999L, testObservationWrapper);
        });
        assertEquals("Encounter with the given id does not exist", exception.getMessage());
    ...
    public Observation createObservation(Long id, String code, String value, Patient patient, Encounter encounter, LocalDateTime effectiveDateTime) {
    ...
    void addEncounterObservation_WhenObservationsListExists_ShouldAppendToExistingList() {
        Observation existingObservation = Observation.builder()
                .code("TEMP-001")
                .value("98.6")
                .build();
